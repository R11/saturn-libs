...
- **Byte-stream transport abstraction** (`saturn_io_transport_t`
...
SRCS    += $(LIB_SATURN_IO)/src/connect_async.c   # non-blocking connect
SRCS    += $(LIB_SATURN_IO)/src/matchmaking.c     # matchmake helper
```
...
saturn_io_init(&cfg);             /* powers on modem, sets state */
saturn_io_connect();              /* blocks: detect / probe / dial */

/* Send a frame -- library prepends the [LEN_HI][LEN_LO] header. */
static const uint8_t msg[] = { 'h', 'i' };
saturn_io_send(msg, sizeof(msg));

while (1) {
    slSynch();
    saturn_io_poll();             /* call every frame */
}
```
...
while (1) {
    slSynch();
    saturn_io_connect_tick_result_t t = saturn_io_connect_tick();
    if (t == SATURN_IO_TICK_OK) break;
    if (t != SATURN_IO_TICK_IN_PROGRESS) { /* handle error */ break; }
    /* ... render "connecting..." ... */
}
```

That's the whole API surface you need for a basic online game.  See
`include/saturn_io/net.h` for the full API (state reporting,
stats, DCD monitoring, IRQ-driven RX, manual reconnect, TX buffering,
heartbeat pings, advanced AT command access) and
`include/saturn_io/matchmaking.h` for the pre-game matchmaking
helper.
...
saturn-io/
  include/saturn_io/
    ...
    transport.h       # saturn_io_transport_t function-pointer interface
  ...
  src/
    ...
    matchmaking.c     # saturn_io_matchmake()
    saturn_io_internal.h  # private shared header (do not install)
  tools/
