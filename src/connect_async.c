 ...
 *
 * Implements saturn_io_connect_start() + saturn_io_connect_tick().
 * Shares the library singleton (s_net) defined in net.c via
 * saturn_io_internal.h.
 *
 * Design notes
 * ------------
 * The blocking saturn_io_connect() runs a straight-line dial
 ...
 *   saturn_io_modem_probe's MODEM_PROBE_ATTEMPTS. We preserve
 ...
 * - The blocking saturn_io_connect() remains intact. If the user
...
static saturn_io_connect_tick_result_t s_fail_result =
...
static saturn_io_connect_tick_result_t
status_to_tick(saturn_io_status_t s) {
    switch (s) {
    case SATURN_IO_OK:                 return SATURN_IO_TICK_OK;
    case SATURN_IO_ERR_NO_MODEM:       return SATURN_IO_TICK_ERR_NO_MODEM;
    case SATURN_IO_ERR_NO_RESPONSE:    return SATURN_IO_TICK_ERR_NO_RESPONSE;
    case SATURN_IO_ERR_INIT_FAILED:    return SATURN_IO_TICK_ERR_INIT_FAILED;
    case SATURN_IO_ERR_NO_CARRIER:     return SATURN_IO_TICK_ERR_NO_CARRIER;
    case SATURN_IO_ERR_BUSY:           return SATURN_IO_TICK_ERR_BUSY;
    case SATURN_IO_ERR_NO_DIALTONE:    return SATURN_IO_TICK_ERR_NO_DIALTONE;
    case SATURN_IO_ERR_NO_ANSWER:      return SATURN_IO_TICK_ERR_NO_ANSWER;
    case SATURN_IO_ERR_TIMEOUT:        return SATURN_IO_TICK_ERR_TIMEOUT;
    case SATURN_IO_ERR_INVALID_CONFIG: return SATURN_IO_TICK_ERR_INVALID_CONFIG;
    default:                               return SATURN_IO_TICK_ERR_INTERNAL;
    }
}
...
saturn_io_status_t saturn_io_connect_start(void)
{
    ...
    if (s_net.transport) {
         ...
         * SATURN_IO_TICK_OK result in the same call that marks
    ...
    s_fail_result = SATURN_IO_TICK_ERR_INTERNAL;
...
saturn_io_connect_tick_result_t saturn_io_connect_tick(void)
{
    ...
    case ASYNC_DETECT: {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (saturn_uart_detect(&probe)) {
                ...
                saturn_io_report("Async: modem found");
                saturn_io_set_state(SATURN_IO_STATE_PROBING,
                                         SATURN_IO_OK);
                s_async_state = ASYNC_PROBE;
                return SATURN_IO_TICK_IN_PROGRESS;
            }
        }
        saturn_io_report("Async: no modem");
        saturn_io_set_state(SATURN_IO_STATE_ERROR,
                                 SATURN_IO_ERR_NO_MODEM);
        s_async_state = ASYNC_FAILED;
        s_fail_result = SATURN_IO_TICK_ERR_NO_MODEM;
        return s_fail_result;
    }

    case ASYNC_PROBE:
        saturn_io_report("Async: probing baud...");
        /* NOTE: saturn_io_modem_probe() tries 4 bauds with 3
         ...
         * saturn_io_connect() could sit in. Acceptable for
         * typical hardware that responds on the first baud tried. */
        if (s_net.config.advanced.fixed_baud_divisor != 0) {
            mr = saturn_io_modem_probe_fixed(
                &s_net.uart, s_net.config.advanced.fixed_baud_divisor);
        } else {
            mr = saturn_io_modem_probe(&s_net.uart);
        }
        if (mr != SATURN_IO_MODEM_OK) {
            saturn_io_report("Async: no modem response");
            saturn_io_set_state(SATURN_IO_STATE_ERROR,
                                     SATURN_IO_ERR_NO_RESPONSE);
            s_async_state = ASYNC_FAILED;
            s_fail_result = SATURN_IO_TICK_ERR_NO_RESPONSE;
            return s_fail_result;
        }
    ...
    case ASYNC_AT_ATZ:
        saturn_io_report("Async: ATZ");
        mr = saturn_io_modem_command_timeout(
                &s_net.uart, "ATZ", buf, sizeof(buf),
                ASYNC_ITERS_PER_STEP * ASYNC_PROBE_RETRIES);
        if (mr != SATURN_IO_MODEM_OK) {
            saturn_io_set_state(SATURN_IO_STATE_ERROR,
                                     SATURN_IO_ERR_INIT_FAILED);
            s_async_state = ASYNC_FAILED;
            s_fail_result = SATURN_IO_TICK_ERR_INIT_FAILED;
            return s_fail_result;
        }
    ...
    case ASYNC_AT_ATV1:
        ...
        saturn_io_report("Async: modem ready");
        saturn_io_set_state(SATURN_IO_STATE_CONNECTING,
                                 SATURN_IO_OK);
        s_async_state = ASYNC_DIAL;
        return SATURN_IO_TICK_IN_PROGRESS;

    case ASYNC_DIAL: {
        saturn_io_status_t ds;
        if (s_net.config.mode == SATURN_IO_MODE_DIAL) {
            mr = saturn_io_modem_dial(&s_net.uart,
                                           s_net.config.dial_number,
                                           s_net.dial_timeout_iters);
            switch (mr) {
            case SATURN_IO_MODEM_CONNECT:     ds = SATURN_IO_OK; break;
            case SATURN_IO_MODEM_NO_CARRIER:  ds = SATURN_IO_ERR_NO_CARRIER; break;
            case SATURN_IO_MODEM_BUSY:        ds = SATURN_IO_ERR_BUSY; break;
            case SATURN_IO_MODEM_NO_DIALTONE: ds = SATURN_IO_ERR_NO_DIALTONE; break;
            case SATURN_IO_MODEM_NO_ANSWER:   ds = SATURN_IO_ERR_NO_ANSWER; break;
            case SATURN_IO_MODEM_TIMEOUT_ERR: ds = SATURN_IO_ERR_TIMEOUT; break;
            default:                              ds = SATURN_IO_ERR_NO_CARRIER; break;
            }
        } else {
            ...
            mr = saturn_io_modem_command_timeout(
                    ...
                    s_net.dial_timeout_iters);
            if (mr != SATURN_IO_MODEM_OK) {
                ds = SATURN_IO_ERR_INIT_FAILED;
            } else {
                int len = saturn_io_modem_read_line(
                              &s_net.uart, buf, sizeof(buf),
                              s_net.dial_timeout_iters);
                if (len < 0) {
                    ds = SATURN_IO_ERR_TIMEOUT;
                } else {
                    saturn_io_modem_result_t r2 =
                        saturn_io_modem_parse_response(buf);
                    if (r2 == SATURN_IO_MODEM_CONNECT) ds = SATURN_IO_OK;
                    else                                   ds = SATURN_IO_ERR_NO_CARRIER;
                }
            }
        }

        if (ds != SATURN_IO_OK) {
            saturn_io_set_state(SATURN_IO_STATE_DISCONNECTED, ds);
            s_async_state = ASYNC_FAILED;
    ...
    case ASYNC_DONE:
        ...
        if (s_net.state != SATURN_IO_STATE_CONNECTED) {
            saturn_io_set_state(SATURN_IO_STATE_CONNECTED,
                                     SATURN_IO_OK);
            saturn_io_frame_init(&s_net.rx);
            saturn_io_report("Async: connected (transport)");
        }
        return SATURN_IO_TICK_OK;
    ...
    return SATURN_IO_TICK_ERR_INTERNAL;
}
