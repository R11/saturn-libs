...
static int expect(const char* label, saturn_online_status_t got,
...
int main(void) {
    int failures = 0;
    saturn_online_config_t cfg;

    /* NULL callbacks rejected. */
    cfg = (saturn_online_config_t)SATURN_ONLINE_DEFAULTS;
    failures += expect("init rejects NULL on_frame",
                       saturn_online_init(&cfg),
                       SATURN_ONLINE_ERR_INVALID_CONFIG);
    ...
    failures += expect("init rejects second call",
                       saturn_online_init(&cfg),
                       SATURN_ONLINE_ERR_ALREADY_INIT);

    /* saturn_online_shutdown() powers off the modem via SMPC, which
