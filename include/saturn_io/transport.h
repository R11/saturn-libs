/**
 * saturn_io/transport.h - Byte-stream Transport Abstraction
 *
 * Function-pointer interface for networked play. The default
 * saturn-io implementation wires this to the NetLink UART
 ...
 *
 * Starting in Phase 2, `saturn_io_init()` accepts an optional
...
#ifndef SATURN_IO_TRANSPORT_H
#define SATURN_IO_TRANSPORT_H

#include <stdint.h>
#include <stdbool.h>

typedef struct saturn_io_transport {
    /**
...
static inline bool
saturn_io_transport_rx_ready(const saturn_io_transport_t* t)
{
    return (t && t->rx_ready) ? t->rx_ready(t->ctx) : false;
}

static inline uint8_t
saturn_io_transport_rx_byte(const saturn_io_transport_t* t)
{
    return (t && t->rx_byte) ? t->rx_byte(t->ctx) : 0;
}

static inline int
saturn_io_transport_send(const saturn_io_transport_t* t,
...
static inline bool
saturn_io_transport_is_connected(const saturn_io_transport_t* t)
{
    if (!t) return false;
    return t->is_connected ? t->is_connected(t->ctx) : true;
}

#endif /* SATURN_IO_TRANSPORT_H */
