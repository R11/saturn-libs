/**
 * saturn_online/matchmaking.h - simple matchmaking helper
 *
 ...
 * protocol on top of saturn_online_send/on_frame.
 *
 * Wire protocol
 * -------------
 ...
 *   extra_len may be 0.
 *
 * Both frames are wrapped in saturn_online's length-prefixed framing,
 * so the entire frame appears once on each side via on_frame.
 *
 * This helper owns the connection lifecycle itself: it calls
 * saturn_online_init(), saturn_online_connect(), exchange, and
 * saturn_online_shutdown(). Do not hold a separate saturn_online
 * connection open while calling it.
 */

#ifndef SATURN_ONLINE_MATCHMAKING_H
#define SATURN_ONLINE_MATCHMAKING_H

#include <stdint.h>
#include "saturn_online/net.h"
/* saturn_online_transport_t is forward-declared in net.h; pull in the
...
typedef struct {
     ...
     * saturn_online_config_t::transport: NULL = real UART dial,
...
} saturn_online_matchmake_opts_t;
...
} saturn_online_matchmake_result_t;
 ...
 * @return SATURN_ONLINE_OK on success (including "no match" results
 *         -- check result->opponent_id), or an error code.
 *
 * Caller MUST NOT be inside an active saturn_online session when
...
#endif /* SATURN_ONLINE_MATCHMAKING_H */
